...
export class InvoiceComponent implements OnInit, OnDestroy {
  ...
  getData(formData?: any) {
    ...
    this._backendService.getDocs('INVOICE', formData).forEach((res) => {
      // console.log("res")
  ...
  setData(formData: any) {
    ...
    this._backendService.getCheckOutId(bd).forEach((res) => {
      ...
      if (!Res["status"].error_code) {
        formData.checkoutId = this._backendService.getConfig("checkoutBaseAPIUrl") + "/invoice/" + Res["data"].id
      }
    }).then((res) => {
      this.querySubscription = this._backendService.setDoc('INVOICE', formData).then((res: any) => {
        if (res) {
  ...
  updateData(formData: { _id: any; }) {
    this.dataLoading = true;
    this.querySubscription = this._backendService.updateDoc('INVOICE', formData._id, formData).then((res: any) => {
      if (res) {
        this.savedChanges = true;
        this.error = false;
        this.errorMessage = "";
  ...
  getDoc(docId: any) {
    ...
    this.dataLoading = true;
    this.data$ = this._backendService.getDoc('INVOICE', docId);
  ...
  deleteDoc(docId: any) {
    if (confirm("Are you sure want to delete this record ?")) {
      this.dataLoading = true;
      this._backendService.deleteDoc('INVOICE', docId).then((res: any) => {
        if (res) {
          this.error = false;
          this.errorMessage = "";
          this.dataLoading = false;
  ...
  setEmbedCode(docId: any) {
    return this.embedCode = '<iframe width="560" height="315" src="' + this._backendService.getConfig("checkoutBaseEmbedUrl") + '/invoices/' + docId + '" title="Rapyd Payment Solution" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>'
